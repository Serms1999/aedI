...
void procesar_mcuac(DiccionarioCuacs &dic,string comando){ //ok?  ,Cuac &cuac, int &num
	Cuac nuevo;
	nuevo.leer_mcuac();
	dic.insertar(nuevo);
...
void procesar_pcuac(DiccionarioCuacs &dic,string comando){ //ok?   ,Cuac &cuac, int &num
	Cuac nuevo;
	nuevo.leer_pcuac();
	dic.insertar(nuevo);
}
...
void Interprete(DiccionarioCuacs dic,string comando){
	...
	while (cin >> comando && comando != "exit"){
		if (comando == "mcuac" || comando == "pcuac"){
			if (comando == "mcuac") procesar_mcuac(dic,comando);
			else if (comando == "pcuac") procesar_pcuac(dic,comando);
		}
		...
		//else if (comando == "tag") procesar_tag(string comando);
